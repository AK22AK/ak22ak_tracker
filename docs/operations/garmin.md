...
睡眠评分和同步时间。步数是截至最近同步的快照；真实零步与字段缺失分别保存。页面在
今日和日历单日详情显示恢复参考，月历汇总不读取该数据。
...
P5a-2c 在既有最近八周趋势查询中只读 `daily_wellness` 白名单：睡眠总时长、可选评分
和历史日步数按周计算平均并同时展示覆盖天数。当天步数作为未完成快照排除在平均之外；
缺失不补零，真实零步仍参与历史平均。趋势不纳入睡眠分期、目标达标率、Body Battery
或压力，也不标注好坏或推断因果。

AI 最近十四天上下文进一步投影为最小恢复证据，只包含日期、睡眠总时长/评分、步数、
缺失状态和当天步数未完结标记。它不包含睡眠起止、分期、目标步数、同步时间、来源 ID
或 Provider 原始对象，不参与确定性安全判断。记录内容或覆盖变化会推进 AI 上下文
revision 并改变 hash，使已有建议按现有时效机制失效。
