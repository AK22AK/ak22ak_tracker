...
受保护 App 只在首次在线挂载和离线恢复联网时尝试一个前台恢复批次。客户端 60 秒内
合并重复触发；服务端以 30 分钟到期和两分钟租约抑制多页面并发。该租约、游标及状态
...
- activity 与 wellness 的前台恢复每次最多五天，彼此不能抢占或改写状态；wellness
  成功后只精确更新相应今日/单日缓存，不修改当前未包含恢复参考的月摘要。
