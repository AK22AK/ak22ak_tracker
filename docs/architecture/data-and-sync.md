...
- `resumption_assessments` / `resumption_decisions`：结束临时上下文或暂停后的不可变
  接续快照、人工决定和计划时间线并发边界。
- `evaluation_sessions`：阶段/结项评估会话和冻结证据。当前计划格式没有
  显式 `targetDate` 时，结项目标日由服务端从时间线头完整计划的最后任务日期派生；
  该派生值与基础计划、时间线头和证据范围一起冻结。阶段目标必须以后由结构化计划或
  私人版本化策略提供，不能从公开代码猜测。
...
阶段评估创建事件携带通过 `evaluation-session.schema.json` 校验的冻结证据。周证据只
包含计划任务状态、反馈覆盖和日级最高疼痛/最严重已保存安全级别、暂停/受限/降级天数
以及可测训练覆盖；不包含自由文本、处方正文、外部来源内部标识或私人阈值。尚未导入
版本化 `EvaluationPolicy` 时，系统只标记 `needs_policy`，不会自行宣布有效训练周。

评估结果通过 `evaluation_result_recorded` 追加式事件进入私有镜像，并由
...
- 每天自动同步一次，也提供“立即同步”。
- 首次范围：计划开始日期到当前时间。
- 常规手动追赶完成后仍保留既有近期重叠语义；automatic Cron 不使用固定 overlap 起点，
  而从本轮 claim 前各 scope 最近成功时间对应的计划时区日期（含该日）追赶到今天。
  无成功记录时从 startedOn 开始；持久 nextCursor 优先于新的成功时间。
...
- automatic Cron 改为从 claim 前 lastSucceededAt 对应的计划时区日期（含该日）追赶
  到今天；无成功记录时从 startedOn 开始，持久失败/有界批次 cursor 优先。该规则只
  作用于 automatic path，人工常规追赶和 7/14/30 天历史补录不改变。
