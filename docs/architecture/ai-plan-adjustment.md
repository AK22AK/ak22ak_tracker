...
截至 2026-07-17，DeepSeek 官方模型命名正在迁移，旧兼容名称近期计划弃用。
部署时应从官方文档选择当前模型并通过环境变量更新，不能靠重新发布领域规则。
...
- 请求超时、余额不足、限流或服务过载不会重试用户写入，只重试分析任务。
- Provider 响应慢时页面离开不取消已落库的任务；返回后在待处理区显示。
