...
- 根页 Server Component 在 route children 树中声明明确的根 Tab 身份。共享壳穿过 Next
  Flight/segment 包装读取该身份并绑定初始 children；若 pathname 与 children 在并发
  transition 中短暂错位，pathname 只决定当前选中项，目标面板使用对应客户端组件，不能把
  迟到内容永久装入错误 Tab。
