...
P0b 之前，页面把日期选择和顶层导航直接绑定到动态 Server Component。一次日期点击会
重新执行身份校验、当天查询和整月查询，服务器返回前页面没有可提交的新状态。
这不是浏览器主线程被数据库阻塞，而是界面提交被服务端结果延后。

当时的生产测量确认动态路由和数据库往返会让日期与 Tab 出现可感知等待。具体决策背景和
结果记录在
...
- `今日 / 日历 / 计划 / 设置` 位于受保护的共享 Layout。根 Tab 由一个持久客户端 Host
  管理；首次访问后用 React Activity 的 hidden/visible 模式切换，不卸载页面 DOM。
  底部导航、表单草稿、展开状态、滚动位置和查询缓存不随根 Tab 切换重建。
- TanStack Query 管理远端数据的读取、去重、取消、后台刷新和精确失效。
- Dexie 管理需要跨重启保留的私人缓存和离线写入；Query Cache 本身不是权威数据库。
  P2a 启用白名单查询快照，P2b-1 启用任务与反馈两类待同步命令和最小私人安全策略。
...
| `['integrations', key]`           | Garmin、训记、GitHub、AI 状态 | 1 分钟             |
| `['external-records', key, date]` | 某日外部训练与关联状态        | 1 分钟             |
| `['proposals', key, status]`      | 计划调整建议                  | 1 分钟             |
| `['plan-workspace', key]`         | 计划入口、下一训练和待办摘要  | 1 分钟             |
| `['assistant', key]`              | 对话、当前档案和有效记忆      | 30 秒              |
...
- 四个根面板的唯一渲染源是持久 Host 内对应的客户端组件。App Router children 只在非根详情
  路径直接显示；根路径不把 RSC children 复用为初始面板。并发 transition 中 pathname 与
  children 即使短暂错位，也不能把迟到内容永久装入错误 Tab。日历客户端直接从当前 URL 恢复
  `?date=`，因此不依赖被丢弃的根 children 传递日期。
- 已访问的 Tab 立即重新显示保留的 DOM 与 Query Cache，不用全页过渡框隐藏缓存内容。
- 未加载页面先挂载稳定的目标页面壳，再由各内容区独立加载。
- 每个 Tab 保留自己的滚动位置和临时状态；退出登录时清空全部用户状态。
- 今日成功加载后只在浏览器空闲时预取当前月摘要、当天详情和设置状态；预取使用相同
  Query Key，不执行写操作，不循环触发。
- 计划根面板与日历、设置一样延迟加载并由持久 Host 独占；`/plan/*` 详情由 App Router
  渲染。旧 `/trends` 与 `/trends/*` 只做兼容跳转到 `/plan/review` 或对应计划详情，
  不再拥有根面板。

### 写入

P2b-1 已将以下语义用于任务更新和身体反馈追加；来源关联、执行上下文、暂停和接续
仍明确要求联网。

任务与反馈操作采用“仅保存在本机”“正在同步”和“需要处理”三个本机队列状态：

1. 本地生成 UUID 与 idempotency key，立即写入 Dexie。
2. 立刻更新相关日期和月摘要缓存，显示“待同步”，而不是虚假显示“已保存到
   云端”。
3. 在线时提交 API；成功后先以服务器规范响应更新 Query Cache 与 IndexedDB 快照，
   再移除本地队列项。
4. 失败时保留本地记录、错误代码和重试入口。
5. 应用启动、online、visible、focus 和手动操作共用单消费者租约并按创建时间重放。

### 外部训练导入与确认

- 今日任务的“实际完成情况”首先展示已同步的 Garmin 与训记来源，以及待确认的关联
  建议；不把逐项重量输入框作为正常主路径。
- 训记同步到动作、重量、组次和次数后，使用者只需确认“关联到此任务 / 更换任务 /
  与计划无关”。确认关联仍不等于完成任务，完成选框保持独立。
- 没有匹配记录时再提供折叠的手工录入兜底；已经在训记存在的数据不要求重复输入。
- “立即同步”显示本次范围、最近成功时间和来源级错误。刷新保留当前任务展开状态、
  表单草稿和已加载内容，只局部更新外部记录区。
- 设置页提供训记连接、更新凭证、只读验证、最近同步和断开入口。API Key 录入成功后
  只显示掩码和状态，浏览器不能再次读取原值。
...
- 点击日期或冷 Tab 后 100 ms 内出现新的选中状态或稳定目标页面壳。
- 已缓存 Tab 的真实内容在 100 ms 内重新可见，不等待 RSC 或网络，也不显示全页 loading。
- 暖 Tab 返回不重新请求对应聚合 API。
