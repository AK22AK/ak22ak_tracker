...
- 发给 DeepSeek 的上下文移除身份、仓库和原始医疗文档，只包含完成任务所需的
  完整当前计划、近期结构化反馈、用户确认训练、严格白名单外部证据和安全摘要。只有
  已保存、限长、在发送前预览中实际展示的反馈 observation 可以进入请求；训练备注、
  计划来源、外部 Provider raw/ID 和凭证均不进入请求。数据库只保留上下文与响应 hash 等审计
  信息，不保存模型原始响应。
- 康复助手的任务/活动预关联 ID 只在服务端筛选当天证据；发送给模型的关联只保留类型
  和日期。连续对话只发送最近 20 轮成功回复；一次补充历史查询必须是明确范围且不超过
  30 天。原始 MRI/PDF/笔记全文、本机路径、Provider raw/ID、凭证均不得进入模型请求。
- 助手自动记忆只写入受限稳定类别，并且只有查看、改、删自身记忆的能力；记忆操作不能
  写安全策略、康复档案、计划版本、任务或正式身体反馈。正式反馈必须经过可编辑确认卡
  和服务端确定性红黄绿重算。
- 个性化安全策略不是 Secret，但属于私人领域数据：公共仓库只保存通用 Schema 和
  执行器；真实策略通过鉴权 DTO、PostgreSQL 和私有镜像流转。今日快照只保存
  `policyId/version/hash` 引用；P2b-1 将经评审的完整规则单独按数字身份、Tracker、
  版本、hash 和有效期白名单保存，仅供离线本机预判。每次判定记录策略版本/hash，
  客户端值不能取代服务端权威重算。
