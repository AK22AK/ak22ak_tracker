...
- `zhengjing_notes`：完整个人知识入口，保存检查、医嘱、恢复历史和初始计划，并通过
  康复追踪入口引用独立数据私仓。
- `ak22ak_tracker-data`：被个人笔记引用的私有结构化数据扩展，供审计、备份以及获
  授权的 AI 与原始笔记上下文一起读取。
...
- 私有计划版本导入与按目标日期解析有效版本
- 今日任务勾选、跳过、逐项实际训练数据和主观感受
- 月历查看未来计划、历史完成状态和症状反馈
- 每日症状反馈与红黄绿安全分级
- IndexedDB 离线队列与计划缓存边界（待接入页面）
- Garmin、训记、DeepSeek 和 GitHub 镜像适配器边界
- 受约束、需人工确认的计划变更模型
- 数据 Schema、核心同步规则与单元测试
- 稳定命令 ID、任务/事件/outbox 原子幂等写入与数字 ID 身份门禁
